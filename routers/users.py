...
from schemas import PostResponse, Token, UserCreate, UserUpdate, UserPublic, UserPrivate
from datetime import timedelta
from fastapi.security import OAuth2PasswordRequestForm

from sqlalchemy import func
from auth import CurrentUser, create_access_token, hash_password, verify_password, oauth2_scheme, verify_access_token
...
async def login_for_Access_token(
    ...
    # OAuth2PasswordRequestForm uses "username" field but we treat it as emaiol
...
async def delete_user(user_id: int,current_user:CurrentUser, db: Annotated[AsyncSession, Depends(get_db)]):
    ...
    result = await db.execute(select(models.Post).where(models.User.id == user_id))
    user = result.scalars().first()
    if not user:
